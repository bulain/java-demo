...
public class LoginVm implements Serializable {
    private static final long serialVersionUID = -752280566110654287L;

    private Page page = new Page();
    private LoginSearch search = new LoginSearch();
    ...
    private List<Item> listReferenceBoolean;
    ...
    @Command("new")
    @GlobalCommand("global.login.new")
    @NotifyChange({"view"})
    public void newn() {
        Login l = new Login();
    ...
    public void show(@BindingParam("id") Long id) {
        Login temp = loginService.get(id);
        view = formatItem(temp);
    }

    @Command
    @GlobalCommand("global.login.edit")
    @NotifyChange({"view"})
    ...
    @Command
    @GlobalCommand("global.login.destroy")
    ...
    @Wire("#personId")
    private Textbox uiPersonId;
    @Wire("#personName")
    private Bandbox uiPersonName;
    @AfterCompose
    public void afterCompose(@ContextParam(ContextType.VIEW) Component view) {
        Selectors.wireComponents(view, this, false);
    }

    @GlobalCommand("global.login.person.set")
    public void globalSetPerson(@BindingParam("person") Person p) {
        uiPersonId.setValue(String.valueOf(p.getId()));
        uiPersonName.setValue(p.getFirstName());
        uiPersonName.close();
        Events.postEvent(Events.ON_CHANGE, uiPersonId, p.getId());

        Bandpopup bandpopup = (Bandpopup) uiPersonName.getFirstChild();
        List<Component> children = bandpopup.getChildren();
    ...
    @Command
    public void initPerson(@ContextParam(ContextType.TRIGGER_EVENT) OpenEvent event) {
        if (event.isOpen()) {
            Object value = event.getValue();
            Map<String, Object> args = new HashMap<String, Object>();
            args.put("name", value);
            args.put("globalCmd", "global.login.person.set");

            Bandpopup bandpopup = (Bandpopup) uiPersonName.getFirstChild();
            Executions.createComponents("/person/_popupList.zul", bandpopup, args);
        } else {
            Bandpopup bandpopup = (Bandpopup) uiPersonName.getFirstChild();
            List<Component> children = bandpopup.getChildren();
            Iterator<Component> iterator = children.iterator();
    ...
    @Command
    public void enterPerson(@ContextParam(ContextType.TRIGGER_EVENT) KeyEvent event) {
        Bandbox bandbox = (Bandbox) event.getTarget();
        String value = bandbox.getValue();
        uiPersonName.open();
    ...
    private void prepare() {
        listReferenceBoolean = referenceService.findItem(ItemConst.NAME_BOOLEAN, getLanguage());
    ...
    public void setListView(List<LoginView> listView) {
    ...
    public List<Item> getListReferenceBoolean() {
        return listReferenceBoolean;
    }
    ...
    public static class PopupVm implements Serializable {
        ...
        private String[][] tables = new String[][]{{"New", "global.login.new"}, {"Refresh", "global.login.new"},
                {"Edit", "global.login.edit"}, {"Delete", "global.login.destroy"}};
