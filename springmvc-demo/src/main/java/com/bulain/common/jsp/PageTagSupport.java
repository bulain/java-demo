...
public class PageTagSupport extends BodyTagSupport {
    ...
    public int doEndTag() throws JspException {
        Page bean = (Page) pageContext.getRequest().getAttribute("page");
        Integer iPage =  bean.getPage();
        Integer iTotalPage =  bean.getTotalPage();
        ...
        try {
            ...
            if (curr > 1) {
                pageContext.getOut().print(
                        "<a rel='prev' class='prev_page' href='?page=" + (curr - 1) + "'>« Previous</a>");
            } else {
            ...
            while (iter.hasNext()) {
                ...
                if (index == curr) {
                    ...
                    pageContext.getOut().print("<a href='?page=" + index + "'>" + index + "</a>");
                }
            }
            if (curr < total) {
                pageContext.getOut().print(
                        "<a rel='next' class='next_page' href='?page=" + (curr + 1) + "'>Next »</a>");
            } else {
